...
def D1d(var1, var2, kind, mode, resonance) : 
    ...
    if (var1 == "mu" and var2 == "p" and kind == "slab" and mode == "alfven" and resonance == "fine") : 
        
        print "Création du fichier : alfven-fine-slab-Dmup1d.dat"
        ti.write(link_1, "Création du fichier : alfven-fine-slab-Dmup1d.dat")
        myfile22_name = "../output/"+name1+"-"+name2+"-"+name3+"_Dmup1d.dat"
        myfile22 = open(myfile22_name, "w")
        print "Ecriture des données dans le fichier : alfven-fine-slab-Dmup1d.dat"
        ti.write(link_1, "Ecriture des données dans le fichier : alfven-fine-slab-Dmup1d.dat")
        D_alfven = np.zeros((len(P), len(mu), len(pa.data1)))
        t_i = time.clock()
        for k in range(len(P)) : 
            for l in range(len(mu)) : 
                for m in range(len(pa.data1)) : 
                    ...
                    ti.pourcentage([k, l, m], [len(P), len(mu), len(pa.data1)])
                    ti.overwrite(link_1, ti.pourcentage2([k, l, m], [len(P), len(mu), len(pa.data1)], t_f - t_i))
                    D_alfven[k, l, m] = dif.D(m, P[k], mu[l], 'mu', 'p', 'slab', 'alfven', 'fine')
                    myfile22.write(str(D_alfven[k, l, m])+"\t")
                myfile22.write("\n")
