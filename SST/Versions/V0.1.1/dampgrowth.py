...
#
#plt.loglog(k, wi[idfast], lw=2, label='$\\omega_I$', c=color[idfast])
#interval  = [pa.kcmfast[mm], pa.kcpfast[mm]]
#for xc in interval : 
#    plt.axvline(x=xc, color='k', linestyle='--')
#plt.loglog(k, wr[idfast], lw=2, label='$\\omega_R$', c=color[idfast])
##plt.loglog(k, wr[idfast-1], lw=2, label='$\\omega_R$', c=color[idfast-1]) #2nd mode fast ... 
...
###############################################################################
# Amortissement de Farmer-Goldreich -------------------------------------------
###############################################################################
# L, VL représentent respectivement l'échelle d'injection et la vitesse du 
# fluide à l'échelle d'injection
def dampFG(i, mode, L, VL, k) : 
    VA     = pa.VA[i]
    VF     = pa.VF[i]
    nu_n   = pa.nu_n[i]
    VAi    = pa.VAi[i]
    ...
    if (mode == "alfven") : 
        MA = VL/VA
        lA = L*MA**(-3)
        if (MA > 1) : #Super-Alfvenique
            kpar = (-(nu_n + VAi**2/nu_in) + np.sqrt((nu_n + VAi**2/nu_in)**2 + 8*VA*nu_n*lA/xi_n))/(2*nu_n*lA)
            lmin = (kpar*np.sqrt(1 + lA*kpar))**(-1)
            if (lmin**(4./3)*MA/L**(1./3) < k**(-1) and k**(-1) <= L*MA**(-3)) :  
                return VA*MA**(3./2)/(L**(1./2)*k**(-1./2))
            if (k**(-1) > L*MA**(-3)) : 
                return VA*MA/(L**(1./3)*k**(-2./3))
            else : return 0.
        if (MA <= 1) : #Sub-Alfvenique
            kpar = (-(nu_n + VAi**2/nu_in) + np.sqrt((nu_n + VAi**2/nu_in)**2 + 8*VA*nu_n*L*MA**(-4)/xi_n))/(2*nu_n*L*MA**(-4))
            lmin = (kpar*np.sqrt(1 + L*MA**(-4)*kpar))**(-1)
            if (lmin**(4./3)*MA**(4./3)/L**(1./3) < k**(-1) and k**(-1) <= L*MA**4) : 
                return VA*MA**2/(k**(-1./2)*L**(1./2))
            if (L*MA**4 < k**(-1) and k**(-1) < L*MA) : 
                return VA*MA**(8./3)/(k**(-2./3)*L**(1./3))
            if (k**(-1) > L*MA) : 
                if (k**(-1) < L) : 
                    return MA**2*VA/L
                if (k**(-1) > L) : # Ici c'est surtout k^-1 >> L
                    return MA**2*VA*L/k**(-2)
            else : return 0.
    if (mode == "fast") : # Théorie des fasts à revoir !!!! 
        MF = VL/VF
        MA = VL/VA
        lA = L*MA**(-3)
        if (MF > 1) : #Super-Fast
            kdam = lA**(-1./3)*((2*nu_ni*VA**2*(cs**2 + VA**2 - 2*VF**2))/(VF*(xi_n*VA**2*(cs**2 - VF**2) + xi_i*cs**2*VF**2)))**(2./3)
            lmin = kdam**(-1)
            if (lmin**(4./3)*MF/L**(1./3) < k**(-1) and k**(-1) <= L*MF**(-3)) :  
                return VF*MF**(3./2)/(L**(1./2)*k**(-1./2))
            if (k**(-1) > L*MF**(-3)) : 
                return VF*MF**(8./3)/(k**(-2./3)*L**(1./3))
            else : return 0.
        if (MF < 1) : #Sub-Fast
            kdam = L**(-1./3)*((2*nu_ni*VL**2*(cs**2 + VA**2 - 2*VF**2))/(VF*(xi_n*VA**2*(cs**2 - VF**2) + xi_i*cs**2*VF**2)))**(2./3)
            lmin = kdam**(-1)           
            if (lmin**(4./3)*MF**(4./3)/L**(1./3) < k**(-1) and k**(-1) <= L*MF**4) : 
                return VF*MF**2/(k**(-1./2)*L**(1./2))
            if (L*MF**4 < k**(-1) and k**(-1) < L*MF) : 
                return VF*MF**(8./3)/(k**(-2./3)*L**(1./3))
            if (k**(-1) > L*MF) : 
                if (k**(-1) < L) : 
                    return MF**2*VF/L
                if (k**(-1) > L) : # Ici c'est surtout k^-1 >> L
                    return MF**2*VF*L/k**(-2)
            else : return 0.
...
def Ir(i, p, w, k, n, typ, width) :
    ...
    if (width == 'fine' and typ == 'alfven') :
        ...
        X2 = 1 - ((VA/(bf.beta(p)*c)) - (n*bf.omega(i, p)/(bf.beta(p)*c*k*np.cos(pa.theta[i]))))**2
        if (abs(X2 - 1) <= 1) :
            return X1*X2
        else : return 0.
    if (width == 'large' and typ == 'alfven') :
        C1 = abs(damp/(bf.beta(p)*c*k*np.cos(pa.theta[i])))**2
        C2 = abs(- VA/(bf.beta(p)*c) + (n*bf.omega(i, p))/(bf.beta(p)*c*k*np.cos(pa.theta[i])))
...
#        w = ti.read_dispersion(i, idk, 'fast')
        ...
        X1 = np.pi/abs(bf.beta(p)*c*k*np.cos(pa.theta[i]))
        X2 = 1 - (VF/(bf.beta(p)*c*np.cos(pa.theta[i])) - (n*bf.omega(i, p)/(bf.beta(p)*c*k*np.cos(pa.theta[i]))))**2
        if (abs(X2 - 1) <= 1) :
            return X1*X2
        else : return 0.
...
#        w = ti.read_dispersion(i, idk, 'fast')
        ...
        C1 = abs(damp/(bf.beta(p)*c*k*np.cos(pa.theta[i])))**2
        C2 = abs(- VF/(bf.beta(p)*c*np.cos(pa.theta[i])) + (n*bf.omega(i, p))/(bf.beta(p)*c*k*np.cos(pa.theta[i])))
        X1 = C1/damp
        X2 = np.arctan((1 + C2)/np.sqrt(C1)) - np.arctan((-1 + C2)/np.sqrt(C1))
        X3 = (1 + C1 - C2**2)/np.sqrt(C1)
        X4 = C2*np.log((1 + C1 + 2*C2 + C2**2)/(1 + C1 - 2*C2 + C2**2))
...
def A(i, w, k, mode, resonance) :
#    k = kvec[0]
#    idk = kvec[1]
    def L(i, w, k ,n) :
        def ftemp6(p) :
            if (p**4*bf.K(p)*Ir(i, p, w, k, n, mode, resonance)/bf.gamma(p)**2 >= 0) :
                return p**4*bf.K(p)*Ir(i, p, w, k, n, mode, resonance)/bf.gamma(p)**2 # Ici on peut choisir la resonance
            else : return 0.
        LL = math.simpson_log(ftemp6, pa.pcmin[i]*1e-6, pa.pcmax[i]*1e6, 500) #Bornes à redéfinir !!! (0, inf) 
        return LL
    X1 = 2*k*pa.p0[i]**2/(np.pi**2*m_p**2*c*bf.H(pa.pcmin[i]*1e-4, pa.pcmax[i]*1e4))
    return X1*(L(i, w, k, -1) + L(i, w, k, +1))
