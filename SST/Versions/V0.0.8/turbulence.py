...
###############################################################################
# Fonction turbulence 
def turbulent_spec(i, k, mode, resonance) : 
    if (mode == 'alfven') : 
        if (resonance == 'fine') :
            if (dg.indamping_alfven(i, k)[1] == 0.) : 
                return 0.
            else : 
                X1 = 1/dg.indamping_alfven(i, k)[1]
            ...
            if (bf.coupling(k, pa.kdecni[i], pa.kdecin[i]) == 'Couplage_faible') : 
                X3 = 0.75*pa.omega0[i]*(pa.VAi[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'alfven', 'fine')
                return np.sqrt(X1*X2*X3)
            elif (bf.coupling(k,pa.kdecni[i], pa.kdecin[i]) == 'Couplage_fort') : 
                X3 = 0.75*pa.omega0[i]*(pa.VA[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'alfven', 'fine')
                return np.sqrt(X1*X2*X3)
            ...
            X2 = (k**(-1)/pa.W[i])*(bf.H(pa.pcmin[i], pa.pcmax[i])/bf.G(pa.pcmin[i], pa.pcmax[i])*(-pa.gradPCR[i]))
            if (bf.coupling(k, pa.kdecni[i], pa.kdecin[i]) == 'Couplage_faible') : 
                X3 = 0.75*pa.omega0[i]*(pa.VAi[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'alfven', 'large')
                return np.sqrt(X1*X2*X3)
            elif (bf.coupling(k,pa.kdecni[i], pa.kdecin[i]) == 'Couplage_fort') : 
                X3 = 0.75*pa.omega0[i]*(pa.VA[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'alfven', 'large')
                return np.sqrt(X1*X2*X3)
            else : return float('NaN')
    if (mode == 'fast') : 
        ...
        if (resonance == 'fine') : 
            ...
            X2 = (k**(-1)/pa.W[i])*(bf.H(pa.pcmin[i], pa.pcmax[i])/bf.G(pa.pcmin[i], pa.pcmax[i])*(-pa.gradPCR[i]))
            if (bf.coupling(k, pa.kdecni[i], pa.kdecin[i]) == 'Couplage_faible') : 
                X3 = 0.75*pa.omega0[i]*(pa.VFi[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'fast', 'fine')
                return np.sqrt(-X1*X2*X3)
            elif (bf.coupling(k,pa.kdecni[i], pa.kdecin[i]) == 'Couplage_fort') : 
                X3 = 0.75*pa.omega0[i]*(pa.VF[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'fast', 'fine')
                return np.sqrt(-X1*X2*X3)
            else : return float('NaN')
        elif (resonance == 'large') :
            if (damp == 0.) : 
            ...
            X2 = (k**(-1)/pa.W[i])*(bf.H(pa.pcmin[i], pa.pcmax[i])/bf.G(pa.pcmin[i], pa.pcmax[i])*(-pa.gradPCR[i]))
            if (bf.coupling(k, pa.kdecni[i], pa.kdecin[i]) == 'Couplage_faible') : 
                X3 = 0.75*pa.omega0[i]*(pa.VFi[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'fast', 'large')
                return np.sqrt(-X1*X2*X3)
            elif (bf.coupling(k,pa.kdecni[i], pa.kdecin[i]) == 'Couplage_fort') : 
                X3 = 0.75*pa.omega0[i]*(pa.VF[i]*m_p*c/(pa.p0[i]*c))*dg.A(i, k, 'fast', 'large')
                return np.sqrt(-X1*X2*X3)
            else : return float('NaN')
