...
def Gprim(i, k) : 
    def ftemp3(n, j) : 
        def ftemp4(p) : 
            return p**3*bf.beta(p)*bf.K(p)*kmaj(i,n,j,p,k,'dirac') #Spectre 3D ou 1D ?? ---> Original
#            return p**1*bf.beta(p)*bf.K(p)*kmaj(i,n,j,p,k,'dirac')#/pa.p0[i] #Spectre 3D ou 1D ?? ---> Modification 1/p**2
