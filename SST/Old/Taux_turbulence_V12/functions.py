...
#        return Gp
    ...
    def A3(k) :# Avec un calcul analytique de l'integrale sur mu - résonnance de lorentz
        ...
        def Gprim(p_c) : 
            ...
            Gp = integ.simpson_precis(f6, p_c, p_max, 1e4)
            return -abs(Gp)
    ...
    def A5(k, mu) :# résonnance de lorentz directionnelle en mu
        def kappanj(n, j) : 
#            gamma_in = disp_1(k)[0]
#            eps = V_A/(beta(p)*c)     
            c1 = (2*mu*k*n*m_p*Omega_0) / (p_0*k**2*(mu**2 - j**2*m_p**2*V_A**2/p_0**2))
            c2 = - (m_p**2*(j**2*k**2*V_A**2 - n**2*Omega_0**2))/(p_0**2*k**2*(mu**2 - j**2*m_p**2*V_A**2/p_0**2))
            ps = - (c1 / 2.) + (1/2.)*np.sqrt(c1**2 - 4*c2)
            kk = (ps**3*np.sqrt(1 - beta(ps)**2)*beta(ps)*K(ps))/abs(mu*k - j*k*m_p*V_A*beta(ps)/p_0)
            if (ps <= p_max and ps >= p_c) : 
                return kk 
            else : return 0
        def Gprim(p_c) : 
            kk = kappanj(-1, 1) + kappanj(1, 1)
            return -abs(kk)
#        print -(1- mu**2)*(p_0**1./(2.*np.pi))*(Gprim(p_c)/H(p_c))
        return -(1- mu**2)*(p_0**1./(2.*np.pi))*(Gprim(p_c)/H(p_c)) #Plutôt vrai à une constante pres (p_0 ou pas ???)
...
#        return A3(k) #----------------
#        return Gprim(ip(k))
    # Taux de croissance à partir d'une résonnance de Lorentz et directionnelle -------
...
#        return A4(k, mu)
    # Taux de croissance à partir d'une résonnance de Dirac et directionnelle -------
    def turbulence_1c(k, mu) : 
        F1 = 1/disp_1(k)[0]
        F3 = (k**(-1)/W)*(H(p_c)/G(p_c))*(-gradP_CR(p))
        if (coupling(k) == 'Couplage_faible') :
            F2 = 0.75*Omega_0*(V_Ai*m_p*c/(p_0*c))*A5(k, mu)
            return np.sqrt(F1*F2*F3)
        elif (coupling(k) == 'Couplage_fort') :
            F2 = 0.75*Omega_0*(V_A*m_p*c/(p_0*c))*A5(k, mu)
            return np.sqrt(F1*F2*F3)
        else : return float('NaN')
#        return A5(k, mu)
...
#        kmax = 1e-10
#        def R(j, n, k_parr) : 
#            gamma_in = disp_1(k_parr)[0]
#            eps = V_A/(beta(p)*c)     
#            kk = gamma_in / (gamma_in**2 + beta(p)**2*c**2*(mu*k_parr - j*k_parr*eps +n*Omega_0/(gamma(p)*beta(p)*c))**2)*abs(k_parr/(gamma(p)*m_p))          
#            return kk 
#        def W(k_perp, p) : 
#            return k_perp*beta(p)*c*np.sqrt(1 - mu**2)/omega(p)
#        def J(n, k_perp) : 
#            def f7(theta) : 
#                return (1/np.pi)*np.cos(n*theta - W(k_perp, p)*np.sin(theta))
#            j = integ.simpson_precis(f7, 0.0001, np.pi, 100)
#            return j 
#        def f8(k_perp, n) :
#            return k_perp*(J(n+1, k_perp)**2 + J(n-1, k_perp)**2)
#        def f10(k_parr, j) : 
#            def f11(k_perp) : 
#                return f8(k_perp, 1)
#            def f12(k_perp) : 
#                return f8(k_perp, -1)
#            I1 = integ.simpson(f11, kmin, kmax)
#            I2 = integ.simpson(f12, kmin, kmax)
##            print I1, I2
#            return turbulence_1(k_parr)**2*(I1*R(j, 1, k_parr)+ I2*R(j, -1, k_parr))
#        def f11(k_parr) : 
#            return f10(k_parr, 1)
#        
##        print -np.pi*omega(p)*(1-mu**2)/B**2*integ.simpson(f11, 2*kmin, kmax)
#        return np.pi*omega(p)*(1-mu**2)/B**2*integ.simpson(f11, 2*kmin, kmax)

    def D_mumu(mu, p) : #Calcul avec une integrale
        kmin = 1e-18
        kmax = 1e-10
        
        def bessel(n, w) : 
#            def f11(theta) : 
#                return (1/np.pi)*np.cos(n*theta - w*np.sin(theta))
#            return integ.newton(f11, 0., np.pi, 10)
            return sp.jv(n, w)
            
        def I(n) : 
            def f12(k_r) : 
                W = k_r*beta(p)*c*np.sqrt(1-mu**2)/omega(p)
                return k_r*(bessel(n+1, W)**2 + bessel(n-1, W)**2)
            return integ.simpson_precis(f12, kmin, kmax, 10000)
        
            
        def J(n, j) : 
            kzi = (n*m_p*Omega_0/(j*gamma(p)*m_p*V_A - p*p_0*mu))
            turbui = turbulence_1(abs(kzi))
            return (j*V_A - beta(p)*c*mu)/n*turbui**2*I(n)
            
        
        SUM = J(-1, 1) - J(1, 1)
        return omega(p)**2*np.pi**2*(1-mu**2)/B*SUM
        
    def D_mumu2(mu, p) : #Calcul analytique
        def J(n, j) : 
            kzi = (n*m_p*Omega_0/(j*gamma(p)*m_p*V_A - p*p_0*mu))
            turbui = turbulence_1(abs(kzi))
            return turbui**2/abs(j*V_A - beta(p)*c*mu)#*abs(kzi)
        
        SUM = J(-1, 1) + J(1, 1)
        return 2*omega(p)**2*np.pi**2/B**2*(1-mu**2)*SUM*B**2
        
    def gyro_1(mu, p) : 
        kzi = abs(omega(p)/(beta(p)*c - V_A))
        turbi = turbulence_1(kzi)
#        turbi = turbulence_1(k(1))*k(p)**(-5/3.) #Pour comparer avec les modes slab de Schlickeiser : la forme du spectre ne change pas (sauf peut être ne 00)
        return 2*np.pi*omega(p)**2*(1-mu**2)*turbi/abs(beta(p)*c - V_A)
    ...
    def gyro_1a(mu, p) : 
        ...
        def J(n, k) : 
            gamma_in = disp_1(k)[0]
            turb = turbulence_1(k)
            F1 = gamma_in*turb**2/(gamma_in**2 + (beta(p)*c*mu*k + V_A*k - omega(p)))
            return F1
         
        def JJ(k) : 
            return J(1, k) + J(-1, k)
            
        SUM = integ.newton(JJ, kmin, kmax, 1e2)
        
        return omega(p)**2*(1 - mu**2)*SUM
        
    # Libre parcours moyen
    def lpm(p) : 
        F1 = (3/8.)*beta(p)*c
        def f22(mu) :
            return (1 - mu**2)**2/D_mumu2(mu, p)
        F2 = integ.simpson_precis(f22, 1e-3, np.pi, 100)
...
#        F2 = turbulence_1(k(p))**(-2)
    ...
    def lpm2(p) : 
        a = V_A/(beta(p)*c)
        kzi = (m_p*Omega_0/(gamma(p)*m_p*V_A - p*p_0*mu))
        turbui = turbulence_1(abs(kzi))
        F1 = -(3/(16*np.pi**2))*(np.log(a+1) + a**2*np.log(1-a)+2*a)/(omega(p)**2*B*turbui**2)
        return F1
...
#        mu = np.linspace(0.3, 0.8, 2)
        ...
        return [mu, tu]
    elif (str_fonction == 'turbulence_1c(k, mu)') : 
#        mu = np.linspace(0.3, 0.8, 2)
#        mu = [1/np.sqrt(2)]
        mu = [1/np.sqrt(2)]
        tu = np.zeros(len(mu))
        for pitch in range(len(mu)) : 
            tu[pitch] = turbulence_1c(x, mu[pitch])
        return [mu, tu]
    # Coefficients de diffusion et libre parcours moyen -----------------------
...
#        ps = [1e-1, 1e0, 1e4]
        ...
        Duu_array = np.zeros(len(ps))
        for momentum in range(len(ps)) : 
            Duu_array[momentum] = gyro_1(x, ps[momentum])/omega(ps[momentum])
#            print D_mumu(x, ps[momentum])
#            Duu_array[momentum] = gamma(ps[momentum])
#            Duu_array[momentum] = gyro_1(x, ps[momentum])
        return [ps, Duu_array]
