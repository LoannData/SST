...
#General expression Xu+2015b formulas (21a) and (21b)
    def omega_R(p) : 
        nu_n = 0
        F1 = (k(p)**2*V_Ai**2 + chi*k(p)*nu_n*nu_ni)**2 + (k(p)**2*nu_n + (1+chi)*nu_ni)*(k(p)**2*nu_n + nu_ni)*k(p)**2*V_Ai**2 
        F2 = chi*k(p)**2*nu_n*nu_ni + k(p)**2*V_Ai**2 + (k(p)**2*nu_n + (1+chi)*nu_ni)**2
        return np.sqrt(F1/F2)

    def gamma_I(p) : 
        nu_n = 0
        F3 = (k(p)**2*nu_n*(k(p)**2*nu_n + (1 + chi)*nu_ni) + k(p)**2*V_Ai**2)*chi*nu_ni
        F4 = 2*(k(p)**2*V_Ai**2 + chi*k(p)**2*nu_n*nu_ni + (k(p)**2*nu_n + (1 + chi)*nu_ni)**2)
        return F3/F4
#------------------------------------------------------------------------------------------
...
# Without nu_n and for =! coupling regimes 
    ...
    for i in range(len(Tkin)) : 
        ...
        w_R[i]      = omega_R(imp2(Tkin[i]))
        w_I[i]      = gamma_I(imp2(Tkin[i]))
        
        w_Rs[i]     = omega_Rs(imp2(Tkin[i]))
        w_Rw[i]     = omega_Rw(imp2(Tkin[i]))
        w_Is[i]     = gamma_Is(imp2(Tkin[i]))
        w_Iw[i]     = gamma_Iw(imp2(Tkin[i]))
    return Tkin, turb, turb_s, turb_w, interval, w_R, w_I, w_Rs, w_Rw, w_Is, w_Iw
    
    
def trace(T, B, f_ion, n_H, Ai, An1, An2, rA1A2, molecular_medium, phase) : 
    Tkin, turb, turb_s, turb_w, interval, w_R, w_I, w_Rs, w_Rw, w_Is, w_Iw = turbulence_rate(T, B, f_ion, n_H, Ai, An1, An2, rA1A2, molecular_medium) 
...
def dispersion(T, B, f_ion, n_H, Ai, An1, An2, rA1A2, molecular_medium, phase) : 
    ...
    plt.loglog(Tkin, w_R, lw=3, label='w_R', ls='-.', c='black')
    plt.loglog(Tkin, w_I, lw=3, label='w_I', ls='-',  c='black')
    for xc in interval : 
        plt.axvline(x=xc, color='k', linestyle='--')
...
dispersion(30  , 4.89e-6, 5e-4, 100, 12, 1, 4, 0.07, 'yes', 'DiM')
