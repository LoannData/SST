...
def D_2mu_mu(n_tot, B, Temp, PCR, gradPCR, title) : 
    ...
    mus = 0.1
    param_p = f.calcul(mus, 'Duu2(mu)', n_tot, B, Temp, PCR, gradPCR)[0]
    print param_p
    ...
    for j in range(len(param_p)) : 
        ...
        for i in range(len(mu)) :
            ...
            D[j][i] = f.calcul(mu[i], 'Duu2(mu)', n_tot, B, Temp, PCR, gradPCR)[1][j]
...
def turbulence_totale(n_tot, B, Temp, PCR, gradPCR, title) : 
    title = where(n_tot, B, Temp, PCR, gradPCR)
    E = np.logspace(-2, 7, 100)
    turb = np.zeros(len(E)) #Turbulence avec la lorentzienne
    turb_old = np.zeros(len(E)) #Turbulence avec le Dirac
    turb_approx = np.zeros(len(E)) #Turbulence dans les régimes approximés
    Y = pr.parametres_temp(n_tot, B, Temp, PCR, gradPCR)
    k1      = Y[10][0]
    k2      = Y[10][1]
    T1      = f.calcul(k1, 'T(k)', n_tot, B, Temp, PCR, gradPCR)
    ...
    for i in range(len(E)) :
        ...
        turb_old[i] = f.calcul(ks, 'turbulence_1(k)', n_tot, B, Temp, PCR, gradPCR)
        turb[i] = f.calcul(ks, 'turbulence_1a(k)', n_tot, B, Temp, PCR, gradPCR)
        turb_approx[i] = f.calcul(ks, 'turbulence_3(k)', n_tot, B, Temp, PCR, gradPCR)
    interval = [T2, T1]
    plt.figure(figsize=(8, 6))
    for xc in interval : 
        plt.axvline(x=xc, color='k', linestyle='--')
    plt.loglog(E, turb_old, c='blue', lw = 3, label='Resonnant')
    plt.loglog(E, turb, c='black', lw = 2, linestyle='-.', label='Non-resonnant')
    plt.loglog(E, turb_approx, 'o', markevery=len(E)/10,  c='black', label='Resonnant + $\\Gamma_I \\ll \\omega_R$')
    plt.title('Alfven turbulence rate in the '+title+' phase'+'\n'+subtitle(n_tot, B, Temp, PCR, gradPCR))
    plt.xlabel('Mass normalised kinetic energy')
    plt.ylabel('Alfven turbulence rate')
    plt.legend(loc='best')
...
print 'Graphe 1 --------------------------------------------------------------'
##################################################################### La turb finale !!!
turbulence_totale(0.2, 5e-6, 6000, 1e-6, 1e-29, 'WNM') 
turbulence_totale(20,  6e-6,   50, 1e-6, 1e-29, 'CNM')
turbulence_totale(100, 4.89e-6, 30, 1e-6, 1e-29, 'DiM')
turbulence_totale(500, 13.9e-6, 10, 1e-6, 1e-29, 'DeM')
turbulence_totale(1e3, 22e-6, 10, 1e-6, 1e-29, 'DeC')
#####################################################################
