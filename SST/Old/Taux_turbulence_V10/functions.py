...
#        return Gp
    ...
    def A3(k) :# Avec un calcul analytique de l'integrale sur mu
...
#                c2 = np.longdouble((n/(p*k))*(m_p/p_0)*Omega_0)
                kk  = (c1/gamma_in)*(-2 + (np.arctan((1+c2)/np.sqrt(c1)) - np.arctan((-1+c2)/np.sqrt(c1)))*((1+c1-c2**2)/np.sqrt(c1)) + c2*np.log((1+c1+2*c2+c2**2)/(1+c1-2*c2+c2**2)))            
#                kk  = np.arctan((1+c2)/np.sqrt(c1)) - np.arctan((-1+c2)/np.sqrt(c1))  #Le pb vient des arctan
            return kk
        ...
        def Gprim(p_c) : 
            def f6(p) : 
#                return p**3*beta(p)*K(p)*kappa(p)
#                return (k/(p))*p**3*beta(p)*K(p)*kappa(p) #Cas, peut être corrigé
                return abs((gamma(p)*m_p/k)**(-1))*p**3*beta(p)*K(p)*kappa(p) #-----------Vrai cas
#                return abs((gamma(p)*m_p/k)**(-1))*p**3*beta(p)*K(p)#*kappa(p)
#                return -p**3*beta(p)*K(p)*1
            Gp = integ.simpson(f6, p_c, p_max)
#            print Gp
            return -abs(Gp)
...
#        return -(2*k*p_0**2*n_0*c/(n_CR(p_c)))*Gprim(p_c) #Ultra faux 
#        return -kappa(ip(k))
...
#        F2 = turbulence_mu(k(p))**2
    ...
    elif (str_fonction == 'turbulence_mu(k)') : 
        return turbulence_mu(x)
    # Coefficients de diffusion et libre parcours moyen -----------------------
